...
public:
	...
	unsigned long long parallelAccumulate(unsigned int lastNumber)
	{
		...
		for (int i = 0; i < numOfThreads - 1; i++)
		{
			...
			future = std::async(std::launch::async, Accumulator::accumulate, std::ref(result), lastNumber, firstNumberOfGroup);
			futuresVector.push_back(std::move(future));
			lastNumber = firstNumberOfGroup - 1;
		}
		future = std::async(std::launch::async, Accumulator::accumulate, std::ref(result), lastNumber, 1);
		futuresVector.push_back(std::move(future));
